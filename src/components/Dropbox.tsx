...
export default function Dropbox() {
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      console.log(`Sending request to ${API_URL}/api/convert`);
      ...
      if (!res.ok) {
        ...
        if (res.status === 0) {
          throw new Error('CORS error: Request blocked. Check server CORS configuration or network connectivity.');
        }
        if (res.status === 404) {
          throw new Error('API endpoint not found. Ensure the server is deployed correctly and the /api/convert route is available.');
        }
      ...
      if (validConverted.length === 0) {
        setErrorMessage('No files were converted successfully. Check file formats and server status.');
      ...
      setErrorMessage(
        msg.includes('timeout')
          ...
          : msg.includes('CORS')
          ? 'CORS error: Unable to connect to server. Ensure the backend allows requests from http://localhost:5173.'
          : msg.includes('not found')
          ? 'Server endpoint not found. Verify that https://nion-rqsx.onrender.com/api/convert is accessible.'
